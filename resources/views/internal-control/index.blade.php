        ...
        <section class="content-panel mb-4" id="emitir-parecer">
            ...
            <form class="internal-control-form" method="POST" action="{{ route('internal-control-reviews.store', $amendment) }}" enctype="multipart/form-data">
                ...
                <div class="internal-control-form-grid">
                    ...
                    <div><label class="form-label" for="municipal_audit_plan_item_id">Item do Plano Anual</label><select class="form-select @error('municipal_audit_plan_item_id') is-invalid @enderror" id="municipal_audit_plan_item_id" name="municipal_audit_plan_item_id"><option value="">Usar referência externa/manual</option>@foreach($auditPlanItems as $item)<option value="{{ $item->id }}" data-phase="{{ $item->phase }}" @selected((string) old('municipal_audit_plan_item_id') === (string) $item->id)>{{ $item->formalReference() }} · {{ $item->phaseLabel() }} · {{ $item->planned_at->format('d/m/Y') }}</option>@endforeach</select><small class="form-hint">O item selecionado será concluído automaticamente por este parecer.</small>@error('municipal_audit_plan_item_id')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    <div><label class="form-label" for="annual_audit_plan_reference">Referência externa/manual</label><input class="form-control @error('annual_audit_plan_reference') is-invalid @enderror" id="annual_audit_plan_reference" name="annual_audit_plan_reference" value="{{ old('annual_audit_plan_reference') }}" placeholder="Ex.: Decreto municipal ou PAA externo" maxlength="255"><small class="form-hint">Obrigatória somente quando não houver item do PAA no TrilhaGov.</small>@error('annual_audit_plan_reference')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
