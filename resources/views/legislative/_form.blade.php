...
<div class="legislative-simple-form">
    <section class="legislative-form-section span-2">
        <div class="legislative-section-heading">
            <span>1</span>
            <div><h2>Pedido principal</h2><p>Informe o mínimo necessário para a Câmara receber a indicação. O sistema confere cota, quantidade e saúde automaticamente.</p></div>
        </div>
        <div class="legislative-fields">
            ...
            @endunless
            <label><span>Valor estimado (R$) <b>*</b></span><input class="form-control @error('estimated_amount') is-invalid @enderror" name="estimated_amount" type="number" min="{{ $htmlMinimumAmount }}" @if($maximumAmount !== null) max="{{ $maximumAmount }}" @endif step="0.01" value="{{ old('estimated_amount', $proposal->estimated_amount ?? ($prefill['estimated_amount'] ?? '')) }}" required>@error('estimated_amount')<small class="invalid-feedback">{{ $message }}</small>@enderror</label>
            <label class="{{ $editing ? 'span-2' : '' }}"><span>Objeto específico <b>*</b></span><textarea class="form-control @error('object') is-invalid @enderror" name="object" rows="3" minlength="20" maxlength="5000" required placeholder="Ex.: Aquisição de equipamento para a unidade de saúde do bairro..." data-auto-health-source>{{ old('object', $proposal->object ?? '') }}</textarea>@error('object')<small class="invalid-feedback">{{ $message }}</small>@enderror</label>
            <label><span>Secretaria ou órgão executor <b>*</b></span><input class="form-control @error('responsible_department') is-invalid @enderror" name="responsible_department" value="{{ old('responsible_department', $proposal->responsible_department ?? ($prefill['responsible_department'] ?? '')) }}" maxlength="255" required placeholder="Ex.: Secretaria de Saúde" @if($institutionSuggestions['departments']->isNotEmpty()) list="legislative-departments" @endif data-auto-department data-institution-source="legislative-departments">@error('responsible_department')<small class="invalid-feedback">{{ $message }}</small>@enderror</label>
            ...
            <label class="legislative-switch health"><input class="form-check-input" name="health_related" type="checkbox" value="1" @checked(old('health_related', $proposal->health_related ?? ($prefill['health_related'] ?? false))) data-auto-health-toggle><span><strong>Destinar para saúde</strong><small>O sistema usa esta marcação para conferir a reserva mínima do vereador.</small></span></label>
        </div>
    </section>
    ...
    <section class="legislative-form-section span-2">
        <div class="legislative-section-heading">
            <span>2</span>
            <div><h2>Destino e justificativa</h2><p>Use linguagem simples: quem recebe, onde será aplicado e qual problema municipal será atendido.</p></div>
        </div>
        <div class="legislative-fields">
    ...
    <details class="legislative-extra-fields span-2">
        ...
        <div class="legislative-fields">
            ...
            <label><span>CNPJ da entidade</span><input class="form-control @error('beneficiary_cnpj') is-invalid @enderror" name="beneficiary_cnpj" value="{{ old('beneficiary_cnpj', $proposal->beneficiary_cnpj ?? '') }}" maxlength="20" data-institution-document-target>@error('beneficiary_cnpj')<small class="invalid-feedback">{{ $message }}</small>@enderror</label>
            ...
            <label><span>Fonte da estimativa</span><input class="form-control @error('estimate_source') is-invalid @enderror" name="estimate_source" value="{{ old('estimate_source', $proposal->estimate_source ?? '') }}" maxlength="255" placeholder="Cotação, contrato similar ou pesquisa simples" data-auto-estimate-source>@error('estimate_source')<small class="invalid-feedback">{{ $message }}</small>@enderror</label>
