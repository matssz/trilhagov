...
@section('content')
    <div class="page-heading institutions-heading">
        <div>
            ...
            <p>Base institucional para secretarias, vereadores, unidades, fornecedores, beneficiários e fiscais.</p>
        </div>
        @if ($canEdit)
    ...
    <div class="metric-strip institution-metrics">
        ...
        <article><span>Secretarias e órgãos</span><strong>{{ $metrics['departments'] }}</strong><small>Base para filtros e responsáveis</small></article>
        <article><span>Vereadores</span><strong>{{ $metrics['councilors'] }}</strong><small>Câmara municipal</small></article>
        <article><span>Fornecedores / beneficiários</span><strong>{{ $metrics['suppliers'] + $metrics['beneficiaries'] }}</strong><small>Prestação, contratos e relatórios</small></article>
    </div>

    @if ($canEdit)
        <section class="collapse {{ $errors->any() ? 'show' : '' }} content-panel institution-create-panel" id="new-institution">
            <div class="content-panel-header"><div><span class="step-index">1</span><h2 class="h5 mb-0">Adicionar cadastro reutilizável</h2></div><span class="legal-badge">Base municipal</span></div>
            <form class="institution-form-grid" method="POST" action="{{ route('municipal-institutions.store') }}" novalidate data-prevent-double-submit>
                ...
                <div><label class="form-label" for="role_title">Cargo / função</label><input class="form-control" id="role_title" name="role_title" value="{{ old('role_title') }}" maxlength="255"></div>
                ...
                <div class="span-2"><label class="form-label" for="legal_name">Razão social / nome completo</label><input class="form-control" id="legal_name" name="legal_name" value="{{ old('legal_name') }}" maxlength="255"></div>
                ...
                <div class="span-2"><label class="form-label" for="address">Endereço</label><input class="form-control" id="address" name="address" value="{{ old('address') }}" maxlength="255"></div>
                <div class="span-2"><label class="form-label" for="notes">Observações</label><input class="form-control" id="notes" name="notes" value="{{ old('notes') }}" maxlength="2000"></div>
    ...
    <form class="contract-filter-bar" method="GET">
        ...
        <label class="contract-filter-search"><span>Pesquisa</span><input class="form-control" name="search" value="{{ $search }}" placeholder="Nome, CNPJ, secretaria ou vínculo"></label>
    ...
    <section class="content-panel institution-list-panel">
        <div class="content-panel-header"><div><span class="page-kicker">Base institucional</span><h2 class="h5 mb-0">Cadastros do Município</h2></div><span class="record-count">{{ $institutions->total() }}</span></div>
                    ...
                    <article class="institution-row {{ $institution->is_active ? '' : 'is-inactive' }}">
                        <div class="institution-main">
                            ...
                            <div>
                                ...
                                <small>{{ $institution->department ?: ($institution->role_title ?: ($institution->legal_name ?: 'Sem vínculo adicional')) }}</small>
                            </div>
                        </div>
                        <div class="institution-contact">
                            <small>Contato</small>
                            <strong>{{ $institution->email ?: 'Não informado' }}</strong>
                        ...
                        @if ($canEdit)
                            <details class="institution-editor">
                                ...
                                <form class="institution-form-grid compact" method="POST" action="{{ route('municipal-institutions.update', $institution) }}" novalidate data-prevent-double-submit>
                                    ...
                                    <div><label class="form-label" for="role_title_{{ $institution->id }}">Cargo / função</label><input class="form-control" id="role_title_{{ $institution->id }}" name="role_title" value="{{ $institution->role_title }}"></div>
